...
public class Puzzle {
	...
	public int computeH1(Node currentState) {
		...
		for(int i=0;i<MAX_ROW;i++)
			for (int j = 0; j < MAX_COL; j++) {
				int tileCurr= currentState.getTile(i,j);
				int tileGoal= currentState.getTile(i,j);
				if (tileCurr!=0){
					if(tileCurr!=tileGoal) output++;
				}
			}
		return output;
	}
	...
	public int computeH2(Node currentState) {
		...
		for(int i=1;i<MAX_ROW*MAX_COL;i++){
			int []cur= currentState.getLocation(i);
			int []goal=goalState.getLocation(i);
			result +=PuzzleUtils.manhattanDistance(cur,goal);
		}
		return result;
	}
